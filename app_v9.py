...
def display_products(conn):
    ...
    if products_data:
        ...
        products_df = pd.DataFrame(products_data, columns=["ID", "Item Name", "Brand", "Type/Subtype", "Size", "Cost", "Quantity", "Expiry Date", "Location", "Image Path"])
...
def main():
    ...
    if 'logged_in' not in st.session_state or not st.session_state['logged_in']:
        ...
        with tab1:
            ...
            if add_product_form(conn):
                # Refresh the products list after a new product is added
