...
def admin_view_by_users(conn):
    """
    Displays the potential cost saved as a bar plot of Item Name and Cost.
    """

    # Fetch products data with item name and cost
    c = conn.cursor()
    c.execute('SELECT item_name, cost FROM products')
    products_data = c.fetchall()

    if products_data:
        # Create a DataFrame for the products data
        products_df = pd.DataFrame(products_data, columns=["Item Name", "Cost"])
        
        # Calculate the potential cost saved
        total_cost_saved = products_df["Cost"].sum()
        st.subheader(f"Total Potential Cost Saved: ${total_cost_saved:.2f}")

        # Create a bar plot
        fig, ax = plt.subplots()
        products_df.groupby('Item Name')['Cost'].sum().plot(kind='bar', ax=ax)
        ax.set_ylabel('Cost')
        ax.set_title('Cost Saved per Item')
        st.pyplot(fig)
    else:
...
def display_products(conn):
    ...
    if not products_data:
        ...
        for product in products_data:
            ...
            with st.expander(f"{item_name} (ID: {product_id})"):
                ...
                st.button("Contact Seller", key=f"contact_{product_id}")  # Placeholder for Contact Seller button
...
def main():
    ...
    if 'logged_in' not in st.session_state or not st.session_state['logged_in']:
        ...
        if st.session_state.get('user_email') == 'srinath.svce@gmail.com':
            # Admin user tabs
            # Swapping the order of "Admin View" and "Add Product" tabs
            tab1, tab2, tab3 = st.tabs(["📦 View Products", "➕ Add Product", "👨‍💼 Admin View"])
            with tab1:
                display_products(conn)
            with tab2:
                if add_product_form(conn, user_name):
                    display_products(conn)
            with tab3:
